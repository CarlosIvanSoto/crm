...
El monorepo aloja un segundo producto para agencias de viajes. Las fases 0 a 3C
están hechas. `apps/travel-api` expone 13 routers y 105 procedimientos.
`apps/travel-app` tiene el shell, seis entidades, el tablero y cinco pantallas de
ajustes.
...
solo las comisiones.** Las otras tres quedan fuera y se listan al final. Cada una
...
La fase se corta en dos rebanadas, igual que la Fase 2 y la Fase 3:

- **4A — datos y API.** El modelo `Commission`, el módulo `commissions` y el
  cierre del pendiente de margen de 3C.
- **4B — la app.** La lista, el panel en la ficha de reserva, los reportes y la
  pantalla de ajustes.
...
| Archivo | Estado |
| --- | --- |
| `packages/travel-auth/src/agency.ts:27-33` | `canSeeMargins` y `canRecordPayment` ya escritos |
| `apps/travel-api/src/currency/conversion.service.ts:82-100` | `itemFields()` congela `baseAmount`, `fxRate`, `fxRateAt` |
| `apps/travel-api/src/dashboard/dashboard.service.ts:105` | El patrón de anular un campo de margen por rol |
| `apps/travel-api/src/payments/` | La plantilla de módulo: dinero colgado de una reserva, ciclo de estado, guarda de rol |
| `apps/travel-api/src/trpc/list-input.ts` | `listInput`, `paginate`, `resolveOrderBy`, `countsByKey`, `archivedFilter` |
| `apps/travel-app/app/(app)/[agency]/suppliers/` | La plantilla de cinco archivos de una entidad de lista |
| `apps/travel-app/components/travel/payments/payments-panel.tsx` | El panel compartido: página y pestaña de ficha con el mismo componente |
| `apps/travel-app/lib/trpc/cache.ts:154-164` | La forma de una entrada que no es `RecordKind` (`payment`) |
| `Supplier.defaultCommissionRate` | Ya existe. Hoy es de solo lectura en `supplier-sheet.tsx` |
...
`apps/travel-app/components/travel/record-sheet/record-stack.ts:12-18` obliga a
agregar una entrada en `fields-entity.ts` y en el mapa `BY_ID` de
`lib/trpc/cache.ts`, porque los dos son `satisfies Record<RecordKind, …>`. Una
comisión no tiene campos personalizados. El panel dentro de la ficha de reserva
basta.
...
Commission[]` en `Booking`, y dos en `User` con los nombres `"CommissionEarner"`
y `"CommissionAuthor"`.
...
| `packages/travel-auth/travel-auth-generate` | El script borra las relaciones inversas de `Organization`. Revisar el diff a mano |
...
| `approve` | `PENDING` → `APPROVED`, escribe `approvedAt` |
...
Las otras tres piezas de la Fase 4 en `plan_01.md:579-591` no entran aquí. Cada
...
- **Tareas y recordatorios.** El modelo `Activity` ya tiene `TASK`, `dueAt` y
  ...
  Falta la interfaz y un cron de recordatorios con `TRAVEL_CRON_SECRET`, que
  falla cerrado si la variable no existe. `rates.controller.ts` es el patrón.
- **Documento de cotización.** Enlace público `/q/<token>` y PDF para el cliente.
  No hay generador de PDF ni ruta anónima en el repositorio. El hash de token de
  `apps/api/src/conversations/conversation-sharing.service.ts` es reusable, pero
  su transporte sigue autenticado.
- **`apps/travel-agent`.** El agente eve propio. Es un app nuevo completo.
  `apps/agent` es la plantilla. Ninguna inteligencia entra a la API.
...
9. UNKNOWN — Si una reserva reparte comisión entre dos asesores. El modelo lo
   permite; ninguna regla lo limita.
