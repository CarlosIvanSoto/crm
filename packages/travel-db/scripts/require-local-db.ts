...
if (!resolved) {
	console.error(
		`\n  ${command} needs TRAVEL_DATABASE_URL, and nothing set it.` +
	...
	[
		...
		resolved.from.endsWith(".env.local")
			? [
					...
					"  production credentials into it. Either remove TRAVEL_DATABASE_URL from",
					"  that file, or pull Vercel's environment somewhere inert:",
					"",
					"    vercel env pull .env.vercel",
				].join("\n")
			: "  Check which file is winning:  grep -m1 TRAVEL_DATABASE_URL .env .env.local",
		"",
...
function resolveDatabaseUrl(): { url: string; from: string } | null {
	...
	if (root) {
		for (const file of [".env.local", ".env"]) {
			...
			try {
				const url = parseEnv(readFileSync(path, "utf8")).TRAVEL_DATABASE_URL;
				if (url) return { url, from: file };
			} catch {}
		}
	}

	const fromEnvironment = process.env.TRAVEL_DATABASE_URL;
