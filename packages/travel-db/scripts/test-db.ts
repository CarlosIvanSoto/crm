...
if (!url) {
	fail([
		"Neither TRAVEL_TEST_DATABASE_URL nor TRAVEL_DATABASE_URL is set.",
...
if (!name.endsWith("_test")) {
	fail([
		`TRAVEL_TEST_DATABASE_URL names "${name}", which does not end in _test.`,
		"The suite refuses anything else, because it writes and deletes rows",
		"across two agencies and an interrupted run leaves them behind.",
	]);
}
...
if (!process.env.TRAVEL_TEST_DATABASE_URL) {
	console.log(
		[
			"",
			"  Add this to .env so the suite finds it:",
			"",
			`    TRAVEL_TEST_DATABASE_URL="${url}"`,
			"",
		].join("\n"),
	);
}
...
function drifted(target: string): boolean {
	const result = spawnSync(
		...
		{ stdio: "ignore", env: { ...process.env, TRAVEL_DATABASE_URL: target } },
	);
...
function migrate(target: string): void {
	const result = spawnSync("prisma", ["migrate", "deploy"], {
		stdio: "inherit",
		env: { ...process.env, TRAVEL_DATABASE_URL: target },
	});
...
function resolve(): string | null {
	const explicit = process.env.TRAVEL_TEST_DATABASE_URL;
	if (explicit) return explicit;

	const live = process.env.TRAVEL_DATABASE_URL;
	if (!live) return null;
