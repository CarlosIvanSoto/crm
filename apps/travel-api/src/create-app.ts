...
import { apiUrl, SESSION_COOKIE_NAME } from "@travel/auth";
	...
	SwaggerModule.setup(
		...
		() => {
			...
			const swaggerConfig = new DocumentBuilder()
				.setTitle("Travel API")
				.setDescription(
					`REST surface of the Travel API — auth, health, the internal cron routes, and a generated REST bridge (under ${REST_BRIDGE_PATH}) for every tRPC procedure.`,
				)
				.setVersion("1.0")
