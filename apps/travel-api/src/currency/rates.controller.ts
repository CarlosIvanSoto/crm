...
export class RatesController {
	...
	@AllowAnonymous()
	@ApiOperation({
		summary: "Refresh exchange rates and fill amounts left pending",
	})
	...
	private async run(authorization?: string) {
		if (!this.secret) {
			this.logger.error({
				message:
					"TRAVEL_CRON_SECRET is not set — refusing to run the rates route.",
			});
		...
		const filled = await this.conversion.fillMissingAllAgencies();
